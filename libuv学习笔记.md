...
libuv**强制**使用异步的，事件驱动的编程风格。它的核心工作是提供一个event-loop，还有基于I/O和其它事件通知的回调函数。它在Windows上使用Proactor，在Linux和MacOS上使用Reactor并模拟Proactor的行为（与 Asio 相同）。
...
这张图很明确的表示了libuv中所有I/O的事件循环处理的过程，其实就是 `uv_run()` 函数执行的过程，它内部是一个while循环，每轮迭代叫做"trick"。

其概念模型是：

![image](https://img2024.cnblogs.com/blog/3077699/202504/3077699-20250416142115350-140600487.png)

明确一点：**libuv表现出来的行为是概念模型：timer -> idle -> prepare -> io callback --> check --> close**，可以看出它是希望我们在 `idle` 句柄做一些周期性的任务，`prepare` 句柄准备I/O的数据/缓冲区，io callback进行I/O，`check` 句柄拿到数据/缓冲区，close关闭不需要的句柄并清理资源。

但在实际实现上的模型是：

```c
int uv_run(uv_loop_t* loop, uv_run_mode mode) {
    int timeout;          // poll 阶段的超时时间
    int r;                // 循环是否存活的标志（非0表示存活）
    int can_sleep;        // 是否允许在 poll 阶段休眠（即无待处理回调且无 idle 句柄）

    // 1. 进入循环前，先判断循环是否存活（是否有活跃句柄或请求）
    r = uv__loop_alive(loop);
    // 如果已经不存活，至少更新一次时间（保持时间戳最新）
    if (!r)
        uv__update_time(loop);

    // 2. 针对 UV_RUN_DEFAULT 模式的特殊处理：
    //    为了向后兼容，在进入 while 循环之前先执行一次定时器回调。使得满足概念模型最先计算超时定时器的要求。
    //    （其他模式下，定时器会在轮询后执行，以保持正确的事件顺序: prepare -> poll cb -> check）
    if (mode == UV_RUN_DEFAULT && r != 0 && loop->stop_flag == 0) {
        uv__update_time(loop);
        uv__run_timers(loop);
    }

    // 3. 主事件循环（只要循环存活且未被 uv_stop() 停止）
    while (r != 0 && loop->stop_flag == 0) {
        // 判断是否可以进入休眠阻塞：
        //   - 待处理队列为空（没有要立即执行的回调）
        //   - idle 句柄列表为空（没有 idle 句柄需要持续触发）
        can_sleep = uv__queue_empty(&loop->pending_queue) &&
                    uv__queue_empty(&loop->idle_handles);

        // ---- 阶段 A：处理待处理的回调（I/O 完成回调等） ----
        uv__run_pending(loop);

        // ---- 阶段 B：执行 idle 句柄（优先级最低） ----
        uv__run_idle(loop);
        ...
        // ---- 阶段 D：计算 poll 的超时时间 ----
        timeout = 0;  // 默认不阻塞（立即返回）
        if ((mode == UV_RUN_ONCE && can_sleep) || mode == UV_RUN_DEFAULT)
            timeout = uv__backend_timeout(loop);  // 根据定时器、空闲情况等计算 poll 的超时时间

        // ---- 阶段 E：阻塞式 I/O 多路复用（poll） ----
        uv__io_poll(loop, timeout);

        // ---- 阶段 F：轮询后，立即处理待处理回调（最多8次，防止饥饿） ----
        ...
        // ---- 阶段 H：执行关闭句柄的回调（uv_close 注册的） ----
        uv__run_closing_handles(loop);

        // ---- 阶段 I：更新时间戳，并执行超时的定时器 ----
        uv__update_time(loop);
        uv__run_timers(loop);

        // ---- 阶段 J：重新检查循环是否还存活（为下一轮准备） ----
        r = uv__loop_alive(loop);

        // 如果是 UV_RUN_ONCE 或 UV_RUN_NOWAIT 模式，只运行一轮就退出
        if (mode == UV_RUN_ONCE || mode == UV_RUN_NOWAIT)
            break;
    }

    // 4. 循环结束后，清除 stop_flag（为下一次运行重置状态）
    if (loop->stop_flag != 0)
        loop->stop_flag = 0;

    // 返回循环是否还存活（0表示不再存活，>0表示仍有活跃句柄）
    return r;
}
```
...
3. 调用待处理的回调函数，如果有待处理的回调函数，则去处理它，如果没有就接着往下运行【注意这里，**其实是上一轮事件循环处理的IO事件的回调deferred到下一轮事件循环执行**】。

4. 执行idle句柄的回调。可以发现idle句柄的回调会在每次循环中被执行，除非 `uv_idle_stop` 了。

5. 执行prepare句柄的回调。可以用于准备IO的数据，或者打印日志什么的。
...
6. 计算轮询超时，在阻塞I/O之前，循环会计算阻塞的时间（保证能有唤醒时刻），并将这个I/O进入阻塞状态（如果可以的话，阻塞超时为0则表示不阻塞），这些是计算超时的规则：
   ...
   5. 如果有任何要关闭的句柄，则超时为0。
   6. 如果以上情况均不匹配，则采用最接近的定时器超时，或者如果没有活动的定时器，则为无穷大（`INT_MAX`）。

   > 事件循环将会被阻塞在 I/O 循环上（例如 `epoll_pwait()`），直到该套接字有 I/O 事件就绪时唤醒这个线程，调用关联的回调函数，然后便可以在 handles 上进行读、写或其他想要进行的操作。

7. 执行check句柄的回调。此时表明I/O已经退出阻塞状态了，那么有可能是可读/写数据，也有可能超时了，此时libuv将在这里检查句柄的回调，如果有可读可写的操作就调用他们对应的回调，当超时了就调用超时的处理（就是一个dispatcher）。
...
- `uv_loop_close()` ：释放所有事件循环相关的资源（不是事件循环结构体本身），而后可以析构事件循环结构体。必须在该事件循环已经停止以及相关的所有句柄被关闭后调用，否则报错 `UV_EBUSY` 。不建议放在某个回调中调用，而是应当在 `uv_run` 返回之后直接使用。

- `uv_run()` ：运行事件循环。一共有3种运行模式：

  - 默认模式 `UV_RUN_DEFAULT`：运行事件循环，直到不再有活动的和引用的句柄或请求为止。**如果调用了 `uv_stop()`，且仍有活动句柄或请求，则返回非0。其他情况下返回 0**。
  - 单次阻塞模式 `UV_RUN_ONCE`：轮询1次I/O，如果此刻没有待处理的回调则进入阻塞等待。完成本轮循环后退出事件循环。如果没有待执行的回调则返回0，否则返回非0（说明你还得在某个时机继续运行事件循环来处理这些本轮没有处理的活跃句柄）【一定轮询1次，这个模式是认为loop中当前一定有事件发生】
  - 单次不阻塞模式 `UV_RUN_NOWAIT`：轮询1次I/O，但如果没有待处理的回调则不会阻塞。或者完成本轮循环后退出事件循环。如果没有待执行的回调则返回0，否则返回非0（说明你还得在某个时机继续运行事件循环来处理这些本轮没有处理的活跃句柄）【最多轮询1次】

- `uv_loop_alive()` ：返回非0表示存在活跃的句柄，或者正在关闭句柄。

- `uv_stop()` ：强制停止事件循环（不是立即，但可以确保在下一次事件循环之前停止）。一般是在某个回调中调用。
...
- `uv_default_loop()` ：一个全局静态的默认事件循环。由于是默认预分配的事件循环，所以如果你引入的三方库中也用了libuv，则不要随便关闭这个默认的事件循环，因为可能三方库用了这个默认的事件循环。这个默认的事件循环也需要调用 `uv_loop_close()` 。此函数不是线程安全的，从下面的源码实现中可以看出。
...
`uv_buf_t` 是一个简单的结构体，用于表示在libuv中流转的数据缓冲区。libuv通过它来完成在Linux平台模拟Proactor（因为比如 `uv_read_start` 的回调函数返回的是 `uv_buf_t*`，libuv帮我们从内核缓冲区中拷贝好了数据到我们指定的用户缓冲区 `uv_buf_t` 中）。
...
- **Request**：一个请求表示一个操作的开始，用于给操作添加信息和数据，并在得到应答时完成该请求（比如调用该请求上的回调）。因此请求的生命周期是短暂的，**通常只维持在一个回调函数的时间（所以应该在回调函数里面释放申请的内存）**。请求通常对应着在句柄上的IO操作（如在TCP连接中发送数据），不过也有不在句柄上的操作（如获取本机地址）。

> 可以粗略理解handle为xcb_xxx_t，而request为xcb_cookie_t

对于libuv中的requests，开发者需要确保在进行异步任务提交时，**通过动态申请的request，要在loop所在线程执行的complete回调函数中释放**。用 `uv_work_t` 举例，代码可参考如下：
...
约定：简单起见，以下本文将统称句柄和请求为句柄。两者不再区分。
...
*流程句柄是我起的名字，表示这类句柄是固定在事件循环的流程中的，且它们都是在轮询(polling)之前处理的，表示libuv框架给我们提供的一些固定调用点。*
...
由于 `ensure_closing` 是被 `uv_walk` 调用的，所以我们不知道 `handle` 的具体类型。只能尽量做好 handle 管理，能主动进行 `uv_close`，而不是等到 `uv_walk` 里被兜底 close（兜底close可能对于某些具有自定义资源分配的handle，光 `free(handle->data` 不够）。
...
uv 已经要退出了，但为了执行前面的 `uv__run_closing_handles`，又触发了 uv_run 的执行。此时假设哪里处理不好，其他线程又往 uv 线程抛了新的 handle 任务，这里的时序问题就可能导致 `uv_walk` 关不干净。所以这里一定要做好状态管理，一旦执行过 `uv_stop`，其他线程也不应该再往 uv 抛任务。
